...
def _auth_session_payload(session: BrowserSession) -> dict:
    """Decode the signed auth-session cookie without logging its contents."""
    raw = None
    try:
    ...
    if not raw:
        return {}
    return _decode_jwt_segment(str(raw).split(".")[0])


def _extract_totp_factor_id(*payloads: dict) -> str:
    """Find the server-issued TOTP factor id in password/session payloads."""
    pending = [payload for payload in payloads if isinstance(payload, (dict, list))]
    while pending:
        value = pending.pop()
        if isinstance(value, dict):
            factor_type = str(value.get("factor_type") or value.get("type") or "").lower()
            if factor_type == "totp":
                factor_id = str(value.get("factor_id") or value.get("id") or "").strip()
                if factor_id:
                    return factor_id
...
def _submit_password(session: BrowserSession, password: str) -> dict:
    """Submit an existing account password without logging the credential."""
    if not password:
        raise ValueError("[Codex] 登录密码为空")
    ...
    if resp.status_code != 200:
        ...
        raise RuntimeError(f"[Codex] 密码验证失败 status={resp.status_code}")
    logger.info("[Codex] 账号密码验证通过")
    return _resp_json(resp)


def _submit_mfa_totp(session: BrowserSession, code: str, factor_id: str) -> dict:
    """Submit a current authenticator-app TOTP for an MFA-enabled account."""
    normalized = str(code or "").strip()
    if len(normalized) != 6 or not normalized.isdigit():
        raise ValueError("[Codex] TOTP 验证码必须是 6 位数字")
    normalized_factor_id = str(factor_id or "").strip()
    if not normalized_factor_id:
        raise ValueError("[Codex] 登录会话缺少 TOTP factor id")
    sentinel_resp = request_sentinel_token(session, "mfa_verify")
    sentinel_header, so_header = build_sentinel_header(session, sentinel_resp, "mfa_verify")
    resp = _post_json(
        session,
        "https://auth.openai.com/api/accounts/mfa/verify",
        {"id": normalized_factor_id, "type": "totp", "code": normalized},
    ...
    if resp.status_code != 200:
        ...
        raise RuntimeError(
            f"[Codex] TOTP 2FA 验证失败 status={resp.status_code}, "
            f"error_code={error_code or 'unknown'}"
        )
    logger.info("[Codex] TOTP 2FA 验证通过")
    return _resp_json(resp)
...
def run_codex_oauth(
    ...
    Args:
        ...
        password: 现有账号密码；与 totp_provider 同时传入时改走密码 + TOTP 登录
        totp_provider: TOTP 获取回调 fn()->code，不记录密钥或验证码
    ...
    password_totp_login = bool(password or totp_provider)
    if password_totp_login and not (password and callable(totp_provider)):
        return _codex_result(status="failed", email=email, message="密码 + TOTP 登录配置不完整")
    ...
    if not password_totp_login and otp_provider is None:
        from core.email_provider import wait_for_otp as otp_provider

    session = BrowserSession(proxy=proxy)
    try:
        logger.info("[Codex] 开始授权（全新 session）")
        ...
        if password_totp_login:
            password_payload = _submit_password(session, str(password))
            human_delay("api")
            factor_id = _extract_totp_factor_id(password_payload, _auth_session_payload(session))
            if not factor_id:
                raise RuntimeError("[Codex] 密码验证成功，但登录会话没有返回 TOTP factor id")
            totp_code = str(totp_provider() or "").strip()
        ...
        if auth_source == "cpa":
            ...
            logger.info(f"[Codex][CPA] 成功：{msg}，本地记录={'已保存' if path else '未保存'}")
            return _codex_result(
                status="success",
                ok=True,
                email=email,
        ...
        if auth_source == "sub2":
            ...
            logger.info(f"[Codex][sub2] 成功：{msg}，本地记录={'已保存' if path else '未保存'}")
            return _codex_result(
                status="success",
                ok=True,
                email=email,
        ...
        logger.info(
            f"[Codex] 成功：plan={id_claims.get('plan_type') or 'unknown'}，凭证已保存"
        )
        return _codex_result(
            status="success",
            ok=True,
        ...
        logger.warning(f"[Codex] 账号已废（{exc.error_code}）")
        return _codex_result(
            status="deactivated",
            email=email,
        ...
        if _is_cpa_callback_reauth_error(exc) and _cpa_reauth_round < 2:
            logger.warning(
                "[Codex][CPA] callback 返回 Timeout waiting for OAuth callback，重新开启第 %s/2 轮 Codex 授权",
                _cpa_reauth_round + 1,
            )
        ...
        logger.warning(f"[Codex] 失败：{type(exc).__name__}: {str(exc)[:200]}")
